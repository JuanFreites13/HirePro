-- =====================================================
-- MIGRACIÓN SEGURA: CREAR TABLA POSTULATIONS
-- =====================================================
-- Ejecutar este script en Supabase SQL Editor

-- 1. CREAR TABLA POSTULATIONS
CREATE TABLE IF NOT EXISTS postulations (
  ...
  -- Constraint único: un candidato solo puede tener una postulación por aplicación
...
DROP TRIGGER IF EXISTS update_postulations_updated_at ON postulations;
...
-- 4. MIGRAR DATOS EXISTENTES (SEGURO)
...
DROP POLICY IF EXISTS "Users can view all postulations" ON postulations;
DROP POLICY IF EXISTS "Users can insert postulations" ON postulations;
DROP POLICY IF EXISTS "Users can update postulations" ON postulations;
DROP POLICY IF EXISTS "Users can delete postulations" ON postulations;
...
-- 7. VERIFICACIÓN
SELECT 
  'Tabla postulations creada exitosamente' AS status, 
  COUNT(*) AS total_postulations,
  COUNT(DISTINCT candidate_id) AS candidatos_unicos,
  COUNT(DISTINCT application_id) AS aplicaciones_con_candidatos
FROM postulations;
